  ...
  return (
    <div className="relative min-h-screen overflow-hidden">
        ...
        <AnimatePresence mode="wait">
              ...
              <div className="grid gap-16 md:grid-cols-2">
                {courses.map((c) => (
                  <button
                    ...
                    className={cn(
                      "card text-left p-24 transition-all hover:-translate-y-1 hover:shadow-artistic",
                      courseId === c.id ? "border-t-[8px] border-t-primary ring-2 ring-primary/30" : "",
                    )}
                    style={courseId === c.id ? undefined : { borderTopColor: c.color, borderTopWidth: 6 }}
                  >
                    <p className="label-caps text-secondary mb-8">{c.icon}</p>
                    <h3 className="display text-22 mb-4">{c.title}</h3>
                    <p className="text-14 text-ink/70 mb-12">{c.tagline}</p>
                    <p className="text-12 font-mono text-ink/50">
                      ...
                      {s.options.map((o) => {
                        ...
                        return (
                          <button
                            ...
                            className={cn(
                              "rounded-md px-16 py-12 text-14 border-2 transition-all",
                              active
                                ? "bg-artistic-gradient text-surface border-transparent shadow-bold"
                      ...
                      {q.options.map((opt, oi) => {
                        ...
                        return (
                          <button
                            ...
                            className={cn(
                              "rounded-md px-16 py-12 text-14 text-left border-2 transition-all",
                              picked
                                ? "bg-primary/10 border-primary text-primary"
                                : "border-ink/10 hover:border-primary",
                            )}
                          >
                            <span className="font-mono text-12 uppercase tracking-widest text-ink/40 mr-8">
          ...
          {step === 3 && roadmap && (
            <Panel key="reveal">
              ...
              <h2 className="display text-36 mb-16">
                Your <span className="bg-artistic-gradient bg-clip-text text-transparent">{roadmap.course_title}</span> roadmap
              </h2>
              <p className="text-ink/60 mb-24 max-w-xl">
              ...
              <div className="space-y-12">
                {roadmap.steps.map((s, i) => (
                  ...
                  >
                    <span className="flex h-12 w-12 shrink-0 items-center justify-center rounded-md bg-artistic-gradient text-surface font-display">
                      {s.position}
                    </span>
                    <div className="flex-1">
                      <p className="display text-18 leading-tight">{s.module_title}</p>
                      <p className="text-12 text-ink/60 mt-4">{s.rationale}</p>
                      <p className="text-12 font-mono text-secondary mt-4">
      ...
      <style>{`
        ...
        .input:focus { outline: none; border-color: #3B82F6; box-shadow: 0 0 0 4px rgba(59,130,246,0.18); }
      `}</style>
    </div>
  );
}
...
function FieldBlock({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <label className="block">
      <span className="label-caps text-secondary block mb-4">{label}</span>
      {children}
    </label>
  );
}
...
function Header({ step }: { step: Step }) {
  ...
  return (
    <div className="mb-32">
      <p className="label-caps text-secondary mb-8">Onboarding</p>
      <h1 className="display text-36 mb-16">Let's calibrate.</h1>
          ...
          <div key={l} className="flex-1">
            <div
              className={cn(
                ...
                i <= step ? "bg-artistic-gradient" : "bg-ink/10",
              )}
            />
            <p
              className={cn(
                "label-caps mt-4",
                i === step ? "text-primary" : "text-ink/40",
              )}
            >
...
function BgBlobs() {
  return (
    <div className="pointer-events-none absolute inset-0 -z-10 overflow-hidden">
      <div className="absolute -top-32 -left-32 h-[400px] w-[400px] rounded-full bg-primary/20 blur-3xl animate-blob-drift" />
      <div className="absolute -bottom-32 -right-16 h-[400px] w-[400px] rounded-full bg-secondary/20 blur-3xl animate-blob-drift" style={{ animationDelay: "3s" }} />
    </div>
  );
}
